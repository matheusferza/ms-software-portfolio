...
export default function Component(props: HeroSection) {
    ...
    return (
        <Section elementId={elementId} colors={colors} backgroundSize={backgroundSize} styles={styles.self}>
            <div className={classNames('hero-shell', mapFlexDirectionStyles(sectionFlexDirection))}>
                <div className={classNames('hero-copy', mapStyles({ textAlign: sectionAlign }))}>
                    {subtitle && (
...
function HeroMedia({ media }) {
    return (
        <div className="hero-media-shell w-full">
            <DynamicComponent {...media} className={classNames({ 'hero-media-image': media.type === 'ImageBlock' })} />
        </div>
    );
}
